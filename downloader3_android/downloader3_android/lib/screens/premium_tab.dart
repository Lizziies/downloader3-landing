...
// 👑 Owner-only Schnellumschalter, um Premium für den eigenen
// eingeloggten Account zu setzen/aufzuheben, ohne extra in den
// Admin-Tab wechseln zu müssen.
Future<void> _toggleOwnerPremium(bool value) async {
final email = st.currentEmail;
if (email == null || st.ownerPassword == null) return;
setState(() => ownerToggleBusy = true);
try {
if (value) {
await st.api.adminGrantPremium(st.ownerPassword!, email, null);
} else {
await st.api.adminRevokePremium(st.ownerPassword!, email);
}
...
if (mounted) setState(() => ownerToggleBusy = false);
}
}
...
title: Text(st.t('premium_owner_toggle_label'),
style: const TextStyle(color: Colors.white)),
value: st.isPremium,
onChanged: ownerToggleBusy ? null : _toggleOwnerPremium,
),
),
],
const SizedBox(height: 20),
